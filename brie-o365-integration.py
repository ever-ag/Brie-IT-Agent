...
def process_email(access_token, email):
    """Process a single email with corrected logic"""
    try:
        ...
        # STEP 1: Always search wiki FIRST (regardless of ticket quality)
...
This message was sent by our 🤖 Brie IT Agent"""
                ...
                if send_email_response(access_token, sender, "Possible Solution Found", solution_body, subject):
                    delete_email(access_token, message_id)
...
This message was sent by our 🤖 Brie IT Agent"""
                ...
                # Claude says sufficient but no wiki solution - let IT team handle
                delete_email(access_token, message_id)
