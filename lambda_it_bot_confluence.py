...
def get_or_create_conversation(user_id, user_name, message_text):
    ...
    try:
        ...
        if not interaction_type:
            interaction_type = 'General Support'
        if not redacted_message:
            redacted_message = 'No message content'
        ...
        item = {
            ...
            'description': str(redacted_message)[:200],
            'outcome': 'In Progress',
            'date': datetime.utcnow().isoformat(),
            'conversation_history': json.dumps([{'timestamp': datetime.utcnow().isoformat(), 'message': str(redacted_message), 'from': 'user'}]),
            'metadata': '{}'
        }
        ...
        if 'Item' in response:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Error parsing it-action-processor response: {e}")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if 'body' in event:
            # Check if body is URL-encoded (button clicks) or JSON (events)
            if 'payload=' in event.get('body', ''):
                # URL-encoded button click
                parsed_body = urllib.parse.parse_qs(event['body'])
                if 'payload' in parsed_body:
                    payload_str = parsed_body['payload'][0]
                    print(f"DEBUG: Raw payload string: {payload_str}")
                    try:
                        payload = json.loads(payload_str)
                    except json.JSONDecodeError as e:
                        print(f"Error parsing button payload: {e}")
                        print(f"Payload string was: '{payload_str}'")
        ...
        # Handle approval_processed callback from approval system
        if event.get('action') == 'approval_processed':
            ...
            if interaction_id:
                # Find conversation by interaction_id (scan since we don't have timestamp)
                conversations_table = boto3.resource('dynamodb').Table('brie-it-helpdesk-bot-interactions')
                ...
                if response['Items']:
                    ...
                    current_history = json.loads(item.get('conversation_history', '[]'))
                    
                    # Add system notification messages to conversation history
                    approval_notification = f"🔔 **IT Approval Required**\nType: {event.get('request_type', 'SSO_GROUP')}\nID: {approval_id}\nUsers: {event.get('user_email', slack_context.get('user_email', 'user'))}\nTarget: {event.get('resource_name', event.get('group_name', 'resource'))}\nRequester: {event.get('requester_email', slack_context.get('user_email', 'user'))}\nUrgency: Normal"
                    
                    current_history.append({
                        'timestamp': datetime.now().isoformat(),
                        'message': approval_notification,
                        'from': 'system'
                    })
                    
                    # Add approval result message
                    if status == 'approved':
                        approval_result = f"✅ **Approval Approved**\nApproval {approval_id} approved by {event.get('approver', event.get('approved_by', 'IT'))}"
                        final_outcome = "Completed"
                    else:
                        approval_result = f"❌ **Approval Denied**\nApproval {approval_id} denied by {event.get('approver', event.get('approved_by', 'IT'))}"
                    ...
                    # Add final result message if provided
                    if result_message:
                        current_history.append({
                            'timestamp': datetime.now().isoformat(),
                    ...
                    completion_message = f"✅ **Request Completed**\nUser: {event.get('user_email', slack_context.get('user_email', 'user'))}\nGroup: {event.get('resource_name', event.get('group_name', 'resource'))}\nAction: {event.get('action_taken', 'Processed')}\nApproved by: {event.get('approver', event.get('approved_by', 'IT'))}"
                    ...
                    # Update conversation with complete history and final outcome
                    conversations_table.update_item(
                        Key={'interaction_id': interaction_id, 'timestamp': timestamp},
                        UpdateExpression='SET outcome = :outcome, conversation_history = :history, last_updated = :updated, awaiting_approval = :awaiting',
                        ExpressionAttributeValues={
                    ...
                    if channel:
                        if status == 'approved':
                            message = f"✅ Your request has been approved and processed!\n\n{result_message}"
                        else:
                            message = f"❌ Your request was denied.\n\n{result_message}"
                        send_slack_message(channel, message)
