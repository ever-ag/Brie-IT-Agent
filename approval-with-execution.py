...
def lambda_handler(event, context):
    """IT Approval System with execution"""
...
def approve_request(action_id, approver_name):
    ...
    # Execute the request - call brie-ad-group-manager
    lambda_client = boto3.client('lambda')
    lambda_client.invoke(
        FunctionName='brie-ad-group-manager',
        Payload=json.dumps({
            'user_email': 'matthew.denecke@dairy.com',
            'group_name': 'SSO AWS Ever.Ag Infra Sandbox Admin',  # Default for SSO requests
            'action': 'add',
...
def deny_request(action_id, approver_name):
    ...
    # Send denial message to user (assuming same channel as bot)
    send_slack_message('C09KB40PL9J', f"❌ Your SSO request has been denied by {approver_name}")
