...
def lambda_handler(event, context):
    """SOP workflow with routing to it-action-processor"""
    ...
    if any(keyword in message for keyword in ['distribution list', 'dl', 'employees']):
        ...
        # Route general IT messages to it-action-processor
        return route_to_it_processor(event)

def route_to_it_processor(event):
    """Route general IT messages to it-action-processor"""
    try:
