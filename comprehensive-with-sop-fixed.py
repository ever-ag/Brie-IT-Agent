...
def lambda_handler(event, context):
    """Comprehensive IT system with SOP workflow integration"""
    ...
    # SOP workflow first - check for specific automation patterns
    if handle_user_selection(message, user_id, channel):
        return {'statusCode': 200, 'body': 'Selection processed'}
    
    # Check for specific SOP requests first (most specific)
    if is_sso_request(message):
        return handle_sso_request(message, user_id, channel)
    elif any(keyword in message for keyword in ['distribution list', 'dl', 'employees']) and not any(perf in message for perf in ['slow', 'performance', 'workspace']):
        return handle_dl_request(message, user_id, channel)
    else:
        # Route to comprehensive IT system for everything else
        return handle_comprehensive_it(message, user_id, channel, event)

def handle_comprehensive_it(message, user_id, channel, event):
    """Handle with comprehensive AI-powered IT system"""
    try:
        # Performance troubleshooting responses
        if any(keyword in message for keyword in ['slow', 'performance', 'workspace', 'laptop']):
...
💡 Need help? Say "create ticket" """
        ...
        response = "👋 I'm here to help with IT support! I can assist with:\n\n• Performance issues\n• Software access requests\n• Distribution list requests\n• General troubleshooting\n\nWhat can I help you with?"
        
        send_slack_message(channel, response)
        return {'statusCode': 200, 'body': 'General help sent'}
        
    except Exception as e:
        print(f"Error in comprehensive IT: {e}")
        return {'statusCode': 500, 'body': 'Error processing request'}

def is_sso_request(message):
    """Check if this is specifically an SSO request - must have both access request AND specific app"""
    # Must contain access request words AND specific application
    access_words = ['add me to', 'give me access to', 'need access to']
    apps = ['clickup', 'salesforce', 'workday']
    
    has_access_request = any(access in message for access in access_words)
    has_specific_app = any(app in message for app in apps)
    
    # Special case for AWS - must be "add me to aws" not just "aws workspace"
    if 'aws' in message:
        return 'add me to aws' in message or 'give me access to aws' in message or 'need access to aws' in message
    
    return has_access_request and has_specific_app
