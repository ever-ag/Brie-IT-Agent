...
def lambda_handler(event, context):
    """IT Approval System using passed channel"""
...
def create_approval_request(event):
    ...
    user_id = event.get('user_id', 'U07NZQZQZQZ')
    channel = event.get('channel', 'D07NZQZQZQZ')  # Get the channel from the request
...
Action ID: {action_id}"""
    ...
    # Store request details including the original channel
    try:
        ...
        table.put_item(Item={
            ...
            'user_id': user_id,
            'channel': channel,  # Store the original channel
            'status': 'pending',
...
def approve_request(action_id, approver_name):
    ...
    # Get request details including original channel
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table('it-actions')
        ...
        if 'Item' in response:
            ...
            original_channel = item.get('channel', 'D07NZQZQZQZ')  # Get the original channel
        else:
            # Fallback values
            user_email = 'matthew.denecke@dairy.com'
            group_name = 'SSO AWS Ever.Ag Infra Sandbox Admin'
            user_id = 'U07NZQZQZQZ'
            original_channel = 'D07NZQZQZQZ'
    except Exception as e:
    ...
    # Execute the request with the original channel
    ...
    lambda_client.invoke(
        ...
        Payload=json.dumps({
            ...
            'action_id': action_id,
            'slackContext': {
                'channel': original_channel,  # Use the original channel
                'user': user_id
            }
        })
    )
...
def deny_request(action_id, approver_name):
    ...
    # Get original channel and send denial there
    try:
        dynamodb = boto3.resource('dynamodb')
        table = dynamodb.Table('it-actions')
        ...
        response = table.get_item(Key={'action_id': action_id})
        if 'Item' in response:
            original_channel = response['Item'].get('channel', 'D07NZQZQZQZ')
        else:
            original_channel = 'D07NZQZQZQZ'
    except Exception as e:
    ...
    send_slack_message(original_channel, f"❌ Your SSO request has been denied by {approver_name}")
