...
try {{
    ...
    # Remove duplicates and output results with correct type
    ...
    if ($uniqueGroups) {{
        ...
        foreach ($group in $uniqueGroups) {{
            # Get full group details to check GroupCategory
            $fullGroup = Get-ADGroup -Identity $group.DistinguishedName -Properties GroupCategory
            if ($fullGroup.GroupCategory -eq 'Distribution') {{
                Write-Output "DISTRIBUTION_LIST|$($group.Name)"
            }} else {{
