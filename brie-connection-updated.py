...
def search_confluence_wiki(query_text):
    ...
    try:
        ...
        # Use more specific search with exact phrase matching for better precision
        ...
        if len(clean_query.split()) > 1:
            ...
            print(f"Trying exact phrase search first: {exact_phrase_query}")
...
def is_internal_team_communication(subject, body):
...
def process_email(access_token, email):
    ...
    try:
        ...
        if is_automated_system_email(subject, body):
            print("Detected automated system email - ignoring (no response, no deletion)")
            log_email_processing(message_id, sender, subject, body, "STEP -1: Automated System", 
                               "Detected automated system email", "Ignored - no action", 
...
This message was sent by our 🤖 Brie IT Agent"""
        ...
        if is_internal_team_communication(subject, body):
        ...
        if is_workspaces_issue(subject, body):
            ...
            workspace = get_user_workspace(sender)
