...
def lambda_handler(event, context):
    """SOP workflow with routing fallback"""
    ...
    if any(keyword in message for keyword in ['distribution list', 'dl', 'employees']):
        ...
        # Route general IT messages to it-action-processor with fallback
        return route_to_it_processor(event, channel)

def route_to_it_processor(event, channel):
    """Route general IT messages to it-action-processor with fallback"""
    try:
        ...
        # Return the response from it-action-processor
        ...
        if channel:
            send_slack_message(channel, """I received your IT support request. Here are your options:

• **Password issues:** Call IT Support at 214-807-0784 (emergencies only)
• **Other IT issues:** Email itsupport@ever.ag
• **Create ticket:** Say "create ticket" for assistance

I'm working to connect you with our full IT support system.""")
        
        return {'statusCode': 200, 'body': 'Fallback message sent'}
